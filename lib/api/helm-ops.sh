...
function save_chart_from_release() {
	...
	if [ $? -eq 0 ]; then
		...
		CHART_DIR=$(dirname -- "$0")/charts/$CHART_NAME-$CHART_VERSION
...
function release_update_values() {
	local msg="usage: helm release update-values [RELEASE_NAME] "
	if [[ $# < 1 ]]; then exit_with_help "$msg"; fi
	save_chart_from_release $1
